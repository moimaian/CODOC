...
class RequirementsInstaller(cast(Any, QDialog)):
    ...
    def _build_ui(self) -> None:
        ...
        self.chk_vina.setChecked(True)
        self.chk_boost = QCheckBox(f"Install Boost {BOOST_VERSION}")
        self.chk_adfr = QCheckBox("Validate ADFRsuite / prepare_receptor")
        self.chk_vinagpu = QCheckBox("Install / Rebuild Vina-GPU 2.1")
        self.special_rows = {
            "vina": self.chk_vina,
            "boost": self.chk_boost,
            "adfr": self.chk_adfr,
            "vinagpu": self.chk_vinagpu,
        }
        tools_layout.addWidget(self.chk_vina, 0, 0, 1, 2)
        tools_layout.addWidget(self.chk_boost, 1, 0, 1, 2)
        tools_layout.addWidget(self.chk_adfr, 2, 0, 1, 2)
        tools_layout.addWidget(self.chk_vinagpu, 3, 0, 1, 2)
